...
int moveFork = 0;
// for gripper, -1 for close, 0 for stay, 1 for open
int moveGripper = 0;
...
bool flipGripper = false;
int gripperSpeed = 30; // 0-90 value, 90 is max. I'd recommend a low speed so you don't break your servo
...
void setup() {
  ...
  gripperServo.write(servoStopSpeed);
...
void getDirection() {
  ...
  int OPEN[8] = {0, 1, 0, 1, 0, 1, 1, 0};   // 16. open gripper
  int CLOSE[8] = {1, 0, 1, 0, 1, 0, 1, 0};  // 17. close gripper
  int FAST[8] = {0, 1, 0, 1, 0, 1, 0, 1};   // 18. go fast
  int SLOW[8] = {0, 1, 1, 0, 0, 0, 1, 1};   // 19. go slow
  int SL[8] = {0, 0, 0, 0, 1, 1, 1 , 1};    // 20. strafe left
  int SR[8] = {1, 1, 1, 1, 0, 0, 0, 0};     // 21. strafe right
  ...
  for(int i = 0; i < 8; i++){
    ...
    if(inputArr[i] == OPEN[i]) cmdArr[15] += 1;
    if(inputArr[i] == CLOSE[i]) cmdArr[16] += 1;
    if(inputArr[i] == FAST[i]) cmdArr[17] += 1;
    if(inputArr[i] == SLOW[i]) cmdArr[18] += 1;
    if(inputArr[i] == SL[i]) cmdArr[19] += 1;
    if(inputArr[i] == SR[i]) cmdArr[20] += 1;
  }
  ...
  // Serial.print(moveFork);
  // Serial.print(", Gripper Movement: ");
  // Serial.println(moveGripper);
...
void moveBot(){
  ...
  driveWheels(wheelSpeeds, currentSpeed);
  driveLift();
  driveGripper();
}

// Addresses motor controllers to drive wheels at caclulated speeds
void driveWheels(float* speeds, float currentSpeed){
  ...
  if(speeds[0] < 0){
    wheelSpeeds[0] = - pow(abs(wheelSpeeds[0]), tune) * currentSpeed;
  } else {
    wheelSpeeds[0] = pow(wheelSpeeds[0], tune) * currentSpeed;
  }
  if(speeds[1] < 0){
    wheelSpeeds[1] = - pow(abs(wheelSpeeds[1]), tune) * currentSpeed;
  } else {
    wheelSpeeds[1] = pow(wheelSpeeds[1], tune) * currentSpeed;
  }
  if(speeds[2] < 0){
    wheelSpeeds[2] = - pow(abs(wheelSpeeds[2]), tune) * currentSpeed;
  } else {
    wheelSpeeds[2] = pow(wheelSpeeds[2], tune) * currentSpeed;
  }
  ...
  SoftPWMSet(M3_PWM, int(abs(wheelSpeeds[2])));
}
...
void driveLift(){
  if(flipM4){
    if(moveFork > 0){
      ...
      SoftPWMSet(LIFT_PWM, 255);
    } else if(moveFork < 0){
      digitalWrite(LIFT_DIR, HIGH);
      SoftPWMSet(LIFT_PWM, 255);
    } else{
      digitalWrite(LIFT_DIR, HIGH);
      SoftPWMSet(LIFT_PWM, 0);
    }
  } else {
    if(moveFork > 0){
      digitalWrite(LIFT_DIR, HIGH);
...
void driveGripper() {
  // Serial.println("Driving Gripper");
  if(flipGripper){
    if(moveGripper > 0){
      gripperServo.write(servoStopSpeed + gripperSpeed);
    } else if(moveGripper < 0){
      gripperServo.write(servoStopSpeed - gripperSpeed);
    } else{

...
int gripperSpeed = 40; // 0-90 value, 90 is max. I'd recommend a low speed so you don't break your servo
...
void getDirection(){
  ...
  int SPEED[8] = {0, 1, 0, 1, 0, 1, 0, 1};  // 18. toggle speed
  ...
  for(int i = 0; i < 8; i++){
    ...
    if(inputArr[i] == SPEED[i]) cmdArr[17] += 1;
  }
  ...
  if (prevCommand != command && toggleSpeed) {
    Serial.println("Yes");
    Serial.println("Yes");
